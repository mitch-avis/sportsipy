...
from tests.integration.test_utils import (
    normalize_rankings_complete,
    normalize_rankings_entries,
)
...
class TestCFPNCAAFRankings:
    ...
    def test_rankings_integration(self, *args, **kwargs):
        ...
        assert normalize_rankings_entries(rankings.current_extended) == normalize_rankings_entries(
            self.results_extended
        )
        assert rankings.current == self.results
        assert normalize_rankings_complete(rankings.complete) == normalize_rankings_complete(
            self.results_complete
        )
