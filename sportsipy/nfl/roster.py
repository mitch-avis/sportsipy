...
_BIRTH_DATE_REGEX = re.compile(r'data-birth="([0-9]{4}-[0-9]{2}-[0-9]{2})"')
...
def _int_property_decorator(func):
    ...
    @wraps(func)
    def wrapper(*args):
        index = args[0]._detailed_stats_index if func.__name__ in DETAILED_STATS else args[0]._index
        prop = func(*args)
        try:
...
class Player(AbstractPlayer):
    ...
    def _parse_season(self, row: PyQuery) -> str | None:
        ...
        season_str = season_str.replace("*", "").replace("+", "")
        if season_str.endswith(" Yrs"):
            return "Career"
        return season_str
    ...
    def _parse_player_information(self, player_info: PyQuery) -> None:
        ...
        # Modern PFR pages no longer include itemprop attributes for these.
        if not self._name:
            name = str(player_info("div#meta h1 span").text() or "")
            if not name:
                name = str(player_info("div#meta h1").text() or "")
            self._name = name or self._name
        if not self._height:
            meta_text = str(player_info("div#meta").text() or "")
            height_match = re.search(r"\b\d-\d{1,2}\b", meta_text)
            if height_match:
                self._height = height_match.group(0)
        if not self._weight:
            meta_text = str(player_info("div#meta").text() or "")
            weight_match = re.search(r"\b\d{2,3}lb\b", meta_text)
            if weight_match:
                self._weight = weight_match.group(0).replace("lb", "")

    def _parse_birth_date(self, player_info: PyQuery) -> None:
        ...
        if not birth_date:
            match = _BIRTH_DATE_REGEX.search(str(player_info))
            if match:
                birth_date = match.group(1)
        self._birth_date = birth_date

    def _pull_player_data(self) -> dict[str, dict[str, str]] | None:
        ...
        if self._season is not None:
            for season in self._season:
                if season == "Career":
                    ...
                    break
                index += 1
        if self._index is None and self._season:
            self._index = 0
    ...
    def __call__(self, requested_season: str = "") -> Player:
        ...
        if not self._detailed_stats_seasons:
            return self
        for detailed_index, season in enumerate(self._detailed_stats_seasons):
            if season == requested_season:
    ...
    def birth_date(self) -> str | None:
        ...
        return None if self._birth_date is None else str(self._birth_date)
    ...
    def games_started(self):
        """Return an ``int`` of the number of games the player started."""
        return self._games_started

    @_int_property_decorator
    def approximate_value(self):
        """Return an ``int`` of the player's approximate value which is a.
    ...
    def passing_completion(self):
        """Return a ``float`` of the percentage of passes that were caught by a.
    ...
    def passing_touchdown_percentage(self):
        """Return a ``float`` of the percentage of total passes that are.
    ...
    def interception_percentage(self):
        """Return a ``float`` of the percentage of passes the player throws that.
    ...
    def passing_yards_per_attempt(self):
        """Return a ``float`` of the number of yards gained per passing attempt."""
        return self._passing_yards_per_attempt

    @_float_property_decorator
    def adjusted_yards_per_attempt(self):
        """Return a ``float`` of the adjusted number of yards gained per passing.
    ...
    def yards_per_completed_pass(self):
        """Return a ``float`` of the number of yards gained per completed pass."""
        return self._yards_per_completed_pass

    @_float_property_decorator
    def yards_per_game_played(self):
        """Return a ``float`` of the number of passing yards gained per gamed."""
    ...
    @_float_property_decorator
    def espn_qbr(self):
        """Return a ``float`` of the player's Total Quarterback Rating according.
    ...
    def yards_lost_to_sacks(self):
        """Return an ``int`` of the number of yards lost as a result of sacks."""
        return self._yards_lost_to_sacks

    @_float_property_decorator
    def net_yards_per_pass_attempt(self):
        """Return a ``float`` of the net yards gained per pass attempt, equal to.
    ...
    def adjusted_net_yards_per_pass_attempt(self):
        """Return a ``float`` of the adjusted net yards gained per pass attempt,.
    ...
    def sack_percentage(self):
        """Return a ``float`` of the percentage of times sacked during a passing.
    ...
    def fourth_quarter_comebacks(self):
        """Return an ``int`` of the number of times the player has lead a team to.
    ...
    def game_winning_drives(self):
        """Return an ``int`` of the number of times the player has lead a drive.
    ...
    def yards_per_attempt_index(self):
        """Return an ``int`` comparing players by the average number of yards.
    ...
    def net_yards_per_attempt_index(self):
        """Return an ``int`` comparing players by the net average yards gained.
    ...
    def adjusted_yards_per_attempt_index(self):
        """Return an ``int`` comparing players by the average adjusted yards.
    ...
    def adjusted_net_yards_per_attempt_index(self):
        """Return an ``int`` comparing players by the net average adjusted yards.
    ...
    def completion_percentage_index(self):
        """Return an ``int`` comparing players by their passing completion.
    ...
    def touchdown_percentage_index(self):
        """Return an ``int`` comparing players by the percentage of their passes.
    ...
    def sack_percentage_index(self):
        """Return an ``int`` comparing players by the percentage of plays that.
    ...
    def passer_rating_index(self):
        """Return an ``int`` comparing players by their quarterback rating where.
    ...
    def rush_yards_per_game(self):
        """Return a ``float`` of the average number of rushing yards gained per.
    ...
    def rush_attempts_per_game(self):
        """Return a ``float`` of the average number of rushing attempts the.
    ...
    def first_downs_rushing(self):
        """Return an ``int`` of the number of first downs the player has gained.
    ...
    def rush_yards_before_contact(self):
        """Return an ``int`` of the total number of yards the player has gained.
    ...
    def rush_yards_before_contact_per_attempt(self):
        """Return a ``float`` of the number of yards the player gains on average.
    ...
    def rush_broken_tackles(self):
        """Return an ``int`` of the number of tackles the player broke while.
    ...
    def rush_attempts_per_broken_tackle(self):
        """Return a ``float`` of the number of tackles the player broke per rush."""
    ...
    def receptions_per_game(self):
        """Return a ``float`` of the average number of receptions the player.
    ...
    def receiving_yards_per_game(self):
        """Return a ``float`` of the acerage number of receiving yards the player.
    ...
    def catch_percentage(self):
        """Return a ``float`` of the percentage of passes the player caught while.
    ...
    def receiving_broken_tackles(self):
        """Return an ``int`` of the number of tackles the player has broken after.
    ...
    def receptions_per_broken_tackle(self):
        """Return a ``float`` of the number of receptions the player made per.
    ...
    def dropped_passes(self):
        """Return an ``int`` of the number of times the player has dropped a.
    ...
    def drop_percentage(self):
        """Return a ``float`` of the percentage of passes the player has dropped.
    ...
    @_int_property_decorator
    def touches(self):
        """Return an ``int`` of the combined number of rushing attempts and.
    ...
    def yards_from_scrimmage(self):
        """Return an ``int`` of the total number of yards gained from scrimmage.
    ...
    def rushing_and_receiving_touchdowns(self):
        """Return an ``int`` of the combined number of rushing and receiving.
    ...
    def all_purpose_yards(self):
        """Return an ``int`` of the number of all-purpose yards the player has.
    ...
    def less_than_nineteen_yards_field_goal_attempts(self):
        """Return an ``int`` of the number of field goals the player attempted.
    ...
    def less_than_nineteen_yards_field_goals_made(self):
        """Return an ``int`` of the number of field goals the player made from.
    ...
    def longest_field_goal_made(self):
        """Return an ``int`` of the longest field goal the player made."""
        return self._longest_field_goal_made

    @_float_property_decorator
    def field_goal_percentage(self):
        """Return a ``float`` of the percentage of field goals the player made.
    ...
    def extra_point_percentage(self):
        """Return a ``float`` of the percentage of extra points the player made.
    ...
    def blocked_punts(self):
        """Return an ``int`` of the number of the player's punts that have been.
    ...
    @_int_property_decorator
    def tackles(self):
        """Return an ``int`` of the number of tackles the player made."""
    ...
    @_int_property_decorator
    def safeties(self):
        """Return an ``int`` of the number of safeties the player has scored."""
...
class Roster:
    ...
    def _pull_team_page(self, url: str) -> PyQuery:
        ...
        page = PyQuery(utils.remove_html_comment_tags(page_source))
        # If the page is empty, treat as invalid
        if not page or not page.html() or "Page Not Found (404 error)" in str(page):
            raise ValueError("Invalid team or season.")
        if not page("table#roster") and not page("table#starters"):
            raise ValueError("Invalid team or season.")
        return page
    ...
    def _get_id(self, player: PyQuery) -> str:
        ...
        """
        player_cell = player('td[data-stat="player"]')
        player_id = player_cell.attr("data-append-csv")
        if player_id:
            return str(player_id)

        name_tag = player_cell("a")
    ...
    def _get_name(self, player: PyQuery) -> str:
        ...
        if name:
            return str(name)
        return str(player_cell.text() or "")
    ...
    def _find_players_with_coach(self, year: int | str | None) -> None:
        ...
        rows = page("table#roster tbody tr")
        if not rows:
            rows = page("table#starters tbody tr.full_table")
        for player in rows.items():
